...
type userR struct {
	OwnerCircles        CircleSlice       `boil:"OwnerCircles" json:"OwnerCircles" toml:"OwnerCircles" yaml:"OwnerCircles"`
	MemberCircleMembers CircleMemberSlice `boil:"MemberCircleMembers" json:"MemberCircleMembers" toml:"MemberCircleMembers" yaml:"MemberCircleMembers"`
}
...
var (
	...
	userColumnsWithoutDefault = []string{"id", "name", "deleted_at"}
...
func (o *User) MemberCircleMembers(mods ...qm.QueryMod) circleMemberQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`circle_members`.`member_id`=?", o.ID),
...
// LoadOwnerCircles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerCircles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`circle`),
		qm.WhereIn(`circle.owner_id in ?`, args...),
		qmhelper.WhereIsNull(`circle.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	...
	if singular {
		object.R.OwnerCircles = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OwnerID {
				local.R.OwnerCircles = append(local.R.OwnerCircles, foreign)
				if foreign.R == nil {
					foreign.R = &circleR{}
				}
...
// LoadMemberCircleMembers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadMemberCircleMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`circle_members.member_id in ?`, args...),
		qmhelper.WhereIsNull(`circle_members.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	...
	if singular {
		object.R.MemberCircleMembers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.MemberID {
				local.R.MemberCircleMembers = append(local.R.MemberCircleMembers, foreign)
				if foreign.R == nil {
					foreign.R = &circleMemberR{}
				}
...
// AddOwnerCirclesG adds the given related objects to the existing relationships
...
func (o *User) AddOwnerCircles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Circle) error {
	...
	if o.R == nil {
		o.R = &userR{
			OwnerCircles: related,
		}
	} else {
		o.R.OwnerCircles = append(o.R.OwnerCircles, related...)
	}
...
// AddMemberCircleMembersG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
...
func (o *User) AddMemberCircleMembers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CircleMember) error {
	...
	if o.R == nil {
		o.R = &userR{
			MemberCircleMembers: related,
		}
	} else {
		o.R.MemberCircleMembers = append(o.R.MemberCircleMembers, related...)
	}
