...
func testUserToManyOwnerCircles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, circleDBTypes, false, circleColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, circleDBTypes, false, circleColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.OwnerCircles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerCircles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerCircles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerCircles = nil
	if err = a.L.LoadOwnerCircles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerCircles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyMemberCircleMembers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.MemberCircleMembers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMemberCircleMembers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MemberCircleMembers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.MemberCircleMembers = nil
	if err = a.L.LoadMemberCircleMembers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MemberCircleMembers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpOwnerCircles(t *testing.T) {
	var err error
	...
	foreigners := []*Circle{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, circleDBTypes, false, strmangle.SetComplement(circlePrimaryKeyColumns, circleColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Circle{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerCircles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OwnerCircles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerCircles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerCircles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpMemberCircleMembers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddMemberCircleMembers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.MemberCircleMembers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.MemberCircleMembers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.MemberCircleMembers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `char`, `Name`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`}
	_           = bytes.MinRead
)
